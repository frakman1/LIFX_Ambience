...
- (id)initWithFrame:(CGRect)frame menus:(NSArray *)aMenusArray
{
    ...
    if (self) {
        ...
        // add the "Add" Button.
        _addButton = [[AwesomeMenuItem alloc] initWithImage:[UIImage imageNamed:@"logoback"]
                                       highlightedImage:[UIImage imageNamed:@"logobackhigh"]
                                           ContentImage:[UIImage imageNamed:@"logo"] 
...
- (void)rotateSpinningView2:(UIView *)spiningView stop:(BOOL)stop
{
    [UIView animateWithDuration:0.5 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        ...
        if (finished && !stop && !CGAffineTransformEqualToTransform(spiningView.transform, CGAffineTransformIdentity)) {
            [self rotateSpinningView2:spiningView stop:YES];
        }
    }];
}
 */
...
- (void)rotateSpinningView
{
    [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveLinear animations:^{
        [_addButton setTransform:CGAffineTransformRotate(_addButton.transform, M_PI_2)];
    } completion:^(BOOL finished) {
        if (finished && !CGAffineTransformEqualToTransform(_addButton.transform, CGAffineTransformIdentity)) {
...
- (void)setExpanding:(BOOL)expanding
{
    ...
    /*
    // rotate add button
    float angle = self.isExpanding ? -M_PI*1.0 : 0.0f;
    [UIView animateWithDuration:0.2f animations:^{
        _addButton.transform = CGAffineTransformMakeRotation(angle);
    }];
    */
    ...
    ////////
    CGAffineTransform trans = CGAffineTransformScale(_addButton.transform, 5, 5);
    [UIView animateWithDuration:0.2 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut
                     animations:^{
                         _addButton.transform = trans;
                     } completion:nil];

    
    //_addButton.transform = trans;	// do it instantly, no animation
   // [self addSubview:view];
    // now return the view to normal dimension, animating this tranformation
    [UIView animateWithDuration:0.2 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut
                     animations:^{
                         _addButton.transform = CGAffineTransformScale(_addButton.transform, 0.2, 0.2);
                     }
                     completion:nil];
